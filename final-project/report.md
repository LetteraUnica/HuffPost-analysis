...
## Problem
The problem we considered is to classify news articles based on their headlines and short descriptions.
Furthermore, since we have a big corpus of articles, we would like to find discussion topics in the corpus, show how they relate to the categories of the dataset and their evolution over time.
...
The model was trained for one epoch using the huggingface transformer api, following distilbert-base-uncased [fine tuning guide](https://huggingface.co/docs/transformers/tasks/sequence_classification).
The final performance is a major improvement in the F1 scores on all the previous models, at the cost of way more compute time on GPU.
